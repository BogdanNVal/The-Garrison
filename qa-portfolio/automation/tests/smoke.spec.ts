...
test.describe('The Garrison — smoke (current main)', () => {
  test('home/login pages load', async ({ page }) => {
    const home = await page.goto('/index.php');
    expect(home?.ok()).toBeTruthy();
    await expect(page.getByRole('heading', { name: 'The Garrison.' })).toBeVisible();
  ...
  test('reservation rejects party size above 6', async ({ page }) => {
    ...
    await login(page, email, 'test123');
    await page.goto('/rezervare.php');
    // HTML max=6 blocks normal UI entry; remove it to exercise server-side validation.
    await page.locator('#nr_persoane').evaluate((el: HTMLInputElement) => {
  ...
  test('reservation rejects past dates', async ({ page }) => {
    const email = uniqueEmail('pastdate');
    await signup(page, 'Past Date User', email, 'test123');
    await login(page, email, 'test123');
    await page.goto('/rezervare.php');
    await page.locator('#nr_persoane').fill('2');
    await page.locator('#data_rezervare').evaluate((el: HTMLInputElement) => {
      el.removeAttribute('min');
    });
    await page.locator('#data_rezervare').fill('2020-01-01');
    await page.getByRole('button', { name: 'Submit' }).click();
    await expect(page.getByText('Nu puteti rezerva o data din trecut')).toBeVisible();
  });
  ...
  test('admin can log in and open admin panel', async ({ page }) => {
    ...
    await expect(page.getByRole('link', { name: 'Verifica rezervari' })).toBeVisible();
  });
  ...
  test('livesearch rejects unauthenticated requests', async ({ request }) => {
    const res = await request.post('/livesearch.php', {
      form: { input: 'QA' },
    });
